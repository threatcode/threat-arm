#!/bin/bash
#
# Threat Linux ARM build-script for ThreatTAP
...
imagename=${3:-threat-linux-$1-threattap}
...
desktop="threat-desktop-xfce xfonts-terminus xinput xserver-xorg-video-fbdev"
...
eatmydata debootstrap --foreign --keyring=/usr/share/keyrings/threat-archive-keyring.gpg --include=threat-archive-keyring,eatmydata \
    --components=${components} --arch ${architecture} ${suite} ${work_dir} http://threatcode.github.io/threat
...
git clone --depth 1 https://github.com/wawtechnologies/linux-kernel-3.14.51-catchwire-threattap.git ${work_dir}/usr/src/kernel
cd ${work_dir}/usr/src/kernel
git rev-parse HEAD >${work_dir}/usr/src/kernel-at-commit
patch -p1 --no-backup-if-mismatch <${repo_dir}/patches/threat-wifi-injection-3.14.patch
...
cp ${repo_dir}/kernel-configs/threattap.config .config
cp ${repo_dir}/kernel-configs/threattap.config ${work_dir}/usr/src/threattap.config
...
git clone https://github.com/wawtechnologies/utilities-catchwire-threattap "${basedir}"/utilities
...
#u-boot threattap specific overrides:
...
git clone https://github.com/wawtechnologies/u-boot-2014.04-catchwire-threattap.git "${basedir}"/u-boot
cd "${basedir}"/u-boot
